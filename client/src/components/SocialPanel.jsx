...
function FriendsTab({ onViewProfile, onSendMsg }) {
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', padding:10 }}>
      {friends.length === 0
        ? <div style={{ color:'#3A4828', textAlign:'center', paddingTop:30, fontSize:11 }}>Brak znajomych — kliknij na gracza na mapie by dodać</div>
        : friends.map(f => (
          <div key={f.id} style={{
            display:'flex', alignItems:'center', gap:8, padding:'8px 10px',
            background:'rgba(4,10,22,0.5)', border:'1px solid rgba(59,130,246,0.1)',
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                ...
                <span style={{ color:'#5A6840', fontSize:9 }}>poz.{f.poziom} {f.profesja}</span>
              </div>
              <div style={{ color: f.zalogowany ? '#22C55E' : '#374151', fontSize:8, marginTop:1 }}>{f.zalogowany ? 'Online' : 'Offline'}</div>
            </div>
            <button onClick={() => onViewProfile(f.id)} style={btnStyle('#C8940A')}>Profil</button>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'4px 10px', borderBottom:'1px solid rgba(200,150,32,0.08)', display:'flex', gap:6, alignItems:'center', flexShrink:0 }}>
        ...
        {unread > 0 && <button onClick={markAll} style={{ ...btnStyle('#7A8A5A'), fontSize:8 }}>Odczytaj wszystkie</button>}
      </div>
      ...
      {compose ? (
        <div style={{ padding:12, flex:1, display:'flex', flexDirection:'column', gap:8 }}>
          <div style={{ color:'#E8D070', fontSize:11 }}>Do: <strong>{compose.toName}</strong></div>
          <textarea
            value={text} onChange={e=>setText(e.target.value)}
            placeholder="Treść wiadomości..."
            maxLength={500}
            style={{ flex:1, padding:'8px', background:'rgba(8,13,5,0.6)', color:'#CDD4AA', border:'1px solid rgba(59,130,246,0.2)', borderRadius:4, fontSize:11, outline:'none', resize:'none', fontFamily:'Verdana,sans-serif' }}
          />
          <div style={{ display:'flex', gap:6 }}>
            <button onClick={send} style={btnStyle('#4ADE80')}><IconCheck size={10}/> Wyślij</button>
            <button onClick={()=>setCompose(null)} style={btnStyle('#7A8A5A')}>Anuluj</button>
          </div>
        </div>
      ) : (
        <div style={{ flex:1, overflowY:'auto', padding:8 }}>
          {msgs.length === 0
            ? <div style={{ color:'#3A4828', textAlign:'center', paddingTop:30, fontSize:11 }}>Brak wiadomości</div>
            : msgs.map(m => (
              <div key={m.id} onClick={() => !m.przeczytana && markRead(m.id)}
                style={{
                  padding:'8px 10px', marginBottom:4, borderRadius:6,
                  background: m.przeczytana ? 'rgba(8,13,5,0.4)' : 'rgba(13,26,10,0.7)',
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3 }}>
                  <span style={{ color: m.przeczytana ? '#7A8A5A' : '#E8D070', fontWeight:'bold', fontSize:10 }}>{m.od_nazwa}</span>
                  <span style={{ color:'#3A4828', fontSize:8 }}>{new Date(m.data).toLocaleString('pl-PL',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</span>
                </div>
                <div style={{ color: m.przeczytana ? '#5A6840' : '#CDD4AA', fontSize:10, lineHeight:1.4 }}>{m.tresc}</div>
                <div style={{ display:'flex', gap:4, marginTop:4 }}>
                  <button onClick={e=>{e.stopPropagation();onViewProfile(m.od_id);}} style={{...btnStyle('#C8940A'),fontSize:8}}>Profil</button>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', background:'rgba(8,13,5,0.5)', borderBottom:'1px solid rgba(200,150,32,0.15)', flexShrink:0 }}>
          <span style={{ color:'#C8940A' }}><IconUsers size={14}/></span>
          <span style={{ color:'#E8D070', fontWeight:'bold', fontSize:13 }}>Społeczność</span>
