...
const RANGA_COLOR = { mistrz: '#FCD34D', oficer: '#C8940A', czlonek: '#8A9A6A' };
...
function Btn({ children, onClick, color = '#C8940A', danger, disabled, small, wide }) {
...
function Input({ value, onChange, placeholder, type = 'text', maxLength, rows, style = {} }) {
  const base = {
    ...
    background: 'rgba(8,13,5,0.7)', color: '#CDD4AA',
...
function Label({ children }) {
  return <div style={{ fontSize: 8, color: '#5A6840', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 4 }}>{children}</div>;
}

function SectionTitle({ children, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
      <span style={{ color: '#C8940A', fontSize: 10, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>{children}</span>
      {action}
    </div>
  );
}
...
function ProgressBar({ pct, color = '#C8940A', height = 6 }) {
  return (
...
function GuildChat({ socket }) {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(200,150,32,0.1)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '4px 10px', fontSize: 8, color: '#5A6840', display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(8,13,5,0.3)' }}>
      ...
      <div style={{ height: 100, overflowY: 'auto', padding: '4px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {msgs.length === 0
          ? <div style={{ color: '#3A4828', fontSize: 9, textAlign: 'center', marginTop: 10 }}>Brak wiadomości</div>
          : msgs.map((m, i) => (
            <div key={i} style={{ fontSize: 9, lineHeight: 1.4 }}>
              <span style={{ color: '#FCD34D', fontWeight: 'bold' }}>[{m.kto}]</span>
              {' '}<span style={{ color: '#CDD4AA' }}>{m.tresc}</span>
            </div>
          ))}
        <div ref={bottomRef} />
      </div>
      <form onSubmit={send} style={{ display: 'flex', borderTop: '1px solid rgba(200,150,32,0.06)' }}>
        <input value={input} onChange={e => setInput(e.target.value)}
          ...
          style={{ flex: 1, background: 'transparent', color: '#CDD4AA', border: 'none', padding: '5px 10px', fontSize: 10, outline: 'none', fontFamily: 'Verdana,sans-serif' }} />
        <button type="submit" style={{ padding: '5px 12px', background: 'rgba(74,122,42,0.2)', color: '#C8940A', border: 'none', borderLeft: '1px solid rgba(200,150,32,0.12)', cursor: 'pointer', fontSize: 13 }}>➤</button>
      </form>
    </div>
  );
}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Card color='#C8940A' glow>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <span style={{ color: '#E8D070', fontSize: 16, fontWeight: 'bold' }}>{guild.nazwa}</span>
                {pill(guild.otwarta ? '#22C55E' : '#F87171', guild.otwarta ? '🔓 Otwarta' : '🔒 Zamknięta')}
              </div>
              {guild.opis && <div style={{ color: '#7A8A5A', fontSize: 9, marginTop: 3, fontStyle: 'italic' }}>{guild.opis}</div>}
              <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
                <span style={{ color: '#4ADE80', fontSize: 9 }}>● {online}/{total} online</span>
                <span style={{ color: '#5A6840', fontSize: 9 }}>{RANGA_LABEL[guild.myRanga]}</span>
              </div>
            </div>
          </div>
          {/* EXP bar */}
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ color: '#7A8A5A', fontSize: 8 }}>EXP Gildii · Poz. {guild.lvl}</span>
              <span style={{ color: '#C8940A', fontSize: 8 }}>{fmtNum(guild.gildia_exp || 0)}</span>
            </div>
            <ProgressBar pct={expPct} color='#C8940A' height={7} />
          </div>
        </Card>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
          {[
            ...
            { icon: <IconTrophy size={16} />, label: 'Poziom', value: `Lv${guild.lvl}`, color: '#C8940A' },
            ...
            <div key={label} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(200,150,32,0.1)', borderRadius: 8, padding: '10px 8px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#5A6840', fontSize: 7, textTransform: 'uppercase', letterSpacing: '1px', marginTop: 2 }}>{label}</div>
            </div>
          ))}
        </div>
        ...
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#C8940A', fontSize: 10, fontWeight: 'bold' }}>
          ...
          {editing
            ...
            : <div style={{ color: guild.ogloszenie ? '#CDD4AA' : '#3A4828', fontSize: 10, lineHeight: 1.6, fontStyle: guild.ogloszenie ? 'normal' : 'italic' }}>
        ...
        {guild.activeQuest && (
          <Card color='#22C55E'>
            ...
            <div style={{ color: '#CDD4AA', fontSize: 11, fontWeight: 'bold', marginBottom: 6 }}>{guild.activeQuest.nazwa}</div>
            <ProgressBar pct={questPct} color='#22C55E' />
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, color: '#5A6840', fontSize: 8 }}>
        ...
        {guild.activeWar && (
          <Card color='#EF4444' glow>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center' }}>
              ...
              <div style={{ color: '#5A6840', fontSize: 16, fontWeight: 'bold' }}>VS</div>
              <div>
  ...
  return (
    <Card color='#3B82F6'>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: '#5A6840', fontSize: 9 }}>od {fmtDate(territory.data_zajecia)}</span>
        {income?.territory && (
      ...
      {income?.log?.slice(0, 3).map((l, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 8, color: '#5A6840', marginTop: 3 }}>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '8px 14px', borderBottom: '1px solid rgba(200,150,32,0.08)', flexShrink: 0, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: 2, flex: 1 }}>
          {FILTERS.map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)} style={{
              ...
              color: filter === f.id ? '#E8D070' : '#5A6840',
              borderBottom: filter === f.id ? '2px solid #C8940A' : '2px solid transparent',
        ...
        <div style={{ position: 'relative' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Szukaj..."
            style={{ padding: '4px 8px 4px 24px', background: 'rgba(8,13,5,0.7)', color: '#CDD4AA', border: '1px solid rgba(200,150,32,0.2)', borderRadius: 5, fontSize: 9, outline: 'none', width: 130, fontFamily: 'Verdana,sans-serif' }} />
          <span style={{ position: 'absolute', left: 7, top: 5, color: '#5A6840', fontSize: 11 }}>🔍</span>
        </div>
      </div>

      {/* Column headers */}
      <div style={{ display: 'grid', gridTemplateColumns: '10px 34px 1fr 55px 90px 80px', gap: 6, padding: '5px 14px', borderBottom: '1px solid rgba(200,150,32,0.06)', flexShrink: 0, background: 'rgba(4,8,2,0.4)' }}>
        {['', '', 'Postać', 'Poz.', 'Ranga', 'Akcje'].map((h, i) => (
          <div key={i} style={{ color: '#3A4828', fontSize: 7, textTransform: 'uppercase', letterSpacing: '1px' }}>{h}</div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {members.length === 0
          ? <div style={{ padding: 24, textAlign: 'center', color: '#3A4828', fontSize: 10 }}>Brak wyników</div>
          : members.map(m => {
              ...
              <div key={m.id} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ color: RANGA_COLOR[m.ranga] || '#CDD4AA', fontWeight: 'bold', fontSize: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.nazwa}</span>
                    {m.id === postacId && pill('#C8940A', 'Ty')}
                  </div>
                  <div style={{ color: '#5A6840', fontSize: 8 }}>{m.profesja}</div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ color: '#E8D070', fontSize: 13, fontWeight: 'bold' }}>{m.poziom}</div>
                  <div style={{ color: '#3A4828', fontSize: 7 }}>poziom</div>
                </div>
                  ...
                  {customEdit === m.member_row_id
                    ? <div style={{ display: 'flex', gap: 2 }}>
                        <input value={customVal} onChange={e => setCustomVal(e.target.value)} maxLength={30}
                          style={{ width: 55, padding: '2px 4px', background: 'rgba(8,13,5,0.8)', color: '#CDD4AA', border: '1px solid rgba(200,150,32,0.3)', borderRadius: 3, fontSize: 8, outline: 'none', fontFamily: 'Verdana,sans-serif' }} />
                    ...
                    : <span style={{ color: RANGA_COLOR[m.ranga] || '#CDD4AA', fontSize: 9 }}>{displayRank}</span>
                  }
                </div>
                ...
                <div style={{ display: 'flex', gap: 3, justifyContent: 'flex-end' }}>
                  {guild.myRanga === 'mistrz' && customEdit !== m.member_row_id && (
                    <button onClick={() => { setCustomEdit(m.member_row_id); setCustomVal(m.niestandardowa_ranga || ''); }}
                      title="Ustaw rangę" style={{ padding: '3px 5px', background: 'rgba(200,150,32,0.15)', color: '#C8940A', border: '1px solid rgba(200,150,32,0.3)', borderRadius: 3, cursor: 'pointer' }}>
                      <IconEdit size={10} />
                    </button>
                  )}
                  {canManage && m.ranga === 'czlonek' && guild.myRanga === 'mistrz' && (
                    <button onClick={() => promote(m.id, 'oficer')} title="Awansuj na oficera"
                      style={{ padding: '3px 5px', background: 'rgba(200,150,32,0.15)', color: '#C8940A', border: '1px solid rgba(200,150,32,0.3)', borderRadius: 3, cursor: 'pointer' }}>
                      <IconArrowUp size={10} />
                    </button>
                  )}
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 14px', borderBottom: '1px solid rgba(200,150,32,0.08)', flexShrink: 0 }}>
        {SORTS.map(s => (
          <button key={s.id} onClick={() => setSort(s.id)} style={{
            ...
            borderRadius: 5, cursor: 'pointer', color: sort === s.id ? '#E8D070' : '#5A6840',
        ...
        <div style={{ marginLeft: 'auto', color: '#5A6840', fontSize: 8, alignSelf: 'center' }}>
        ...
        {members.map((m, i) => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: i < 3 ? 'rgba(200,150,32,0.06)' : 'rgba(0,0,0,0.2)', border: `1px solid ${i < 3 ? 'rgba(200,150,32,0.15)' : 'rgba(200,150,32,0.05)'}`, borderRadius: 7 }}>
              {/* Rank */}
              <div style={{ width: 24, textAlign: 'center', fontSize: i < 3 ? 18 : 11, color: '#5A6840', fontWeight: 'bold', flexShrink: 0 }}>
                {i < 3 ? MEDALS[i] : i + 1}
              </div>
              {/* Avatar */}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <span style={{ color: RANGA_COLOR[m.ranga] || '#CDD4AA', fontWeight: 'bold', fontSize: 10 }}>{m.nazwa}</span>
                  <span style={{ color: '#E8D070', fontWeight: 'bold', fontSize: 11 }}>{fmtNum(val)}</span>
                </div>
                <ProgressBar pct={pct} color={i === 0 ? '#FCD34D' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : '#C8940A'} height={5} />
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <Card color='#FCD34D' glow>
        <div style={{ textAlign: 'center', padding: '6px 0 10px' }}>
          ...
          <div style={{ color: '#7A8A5A', fontSize: 9, marginTop: 4, letterSpacing: '2px', textTransform: 'uppercase' }}>💰 Złoto w Skarbcu</div>
        </div>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: canWithdraw ? '1fr 1fr' : '1fr', gap: 10 }}>
        ...
        {canWithdraw && (
          <Card color='#F87171'>
            <SectionTitle>Wypłata <span style={{ color: '#5A6840', fontWeight: 'normal', fontSize: 8 }}>(oficer+)</span></SectionTitle>
            <Label>Kwota</Label>
        ...
        {log.length === 0
          ...
          : log.map(l => (
            <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: 'rgba(0,0,0,0.2)', borderRadius: 6, marginBottom: 4, border: '1px solid rgba(200,150,32,0.06)' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ color: '#CDD4AA', fontSize: 9 }}><span style={{ color: '#C8940A' }}>{l.postac_nazwa}</span> {l.opis ? `— ${l.opis}` : ''}</div>
      ...
      {guild.myRanga === 'mistrz' && (
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ color: '#CDD4AA', fontSize: 10, fontWeight: 'bold' }}>Status rekrutacji</div>
              <div style={{ color: '#5A6840', fontSize: 9, marginTop: 2 }}>Kontroluje czy gracze mogą wysyłać podania</div>
            </div>
      ...
      {canManage && (
        <div>
          ...
          {apps.length === 0
            ...
            : apps.map(a => (
              <Card key={a.id} color='#C8940A'>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: '#E8D070', fontWeight: 'bold', fontSize: 11 }}>{a.postac_nazwa}</div>
                    <div style={{ color: '#5A6840', fontSize: 8, marginTop: 2 }}>{fmtDate(a.data)}</div>
                    {a.tresc && <div style={{ color: '#CDD4AA', fontSize: 9, marginTop: 5, fontStyle: 'italic', borderLeft: '2px solid rgba(200,150,32,0.3)', paddingLeft: 6 }}>"{a.tresc}"</div>}
                  </div>
          ...
          : myApps.map(a => {
            ...
            return (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: 'rgba(0,0,0,0.2)', borderRadius: 6, marginBottom: 4, border: '1px solid rgba(200,150,32,0.06)' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ color: '#CDD4AA', fontSize: 9 }}>{a.gildia_nazwa}</div>
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {guild.activeWar && (
        <Card color='#EF4444' glow>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center', marginBottom: 10 }}>
            ...
            <div style={{ color: '#5A6840', fontSize: 14, fontWeight: 'bold' }}>VS</div>
            <div>
              <div style={{ color: '#60A5FA', fontWeight: 'bold', fontSize: 11 }}>{guild.activeWar.nazwa_broniacej}</div>
      ...
      {guild.myRanga === 'mistrz' && !guild.activeWar && (
        <Card color='#EF4444'>
          <SectionTitle><IconSword size={11} /> Wypowiedz Wojnę <span style={{ color: '#5A6840', fontWeight: 'normal', fontSize: 8 }}>(koszt 500g)</span></SectionTitle>
      ...
      <div>
        ...
        {relations.map(r => {
          ...
          return (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: isAlly ? 'rgba(74,122,42,0.08)' : 'rgba(239,68,68,0.06)', border: `1px solid ${isAlly ? 'rgba(74,122,42,0.2)' : 'rgba(239,68,68,0.15)'}`, borderRadius: 6, marginBottom: 5 }}>
              <span style={{ fontSize: 18 }}>{isAlly ? '🤝' : '⚔'}</span>
              <div style={{ flex: 1 }}>
                <div style={{ color: '#CDD4AA', fontWeight: 'bold', fontSize: 10 }}>{other}</div>
        ...
        <Card>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <select value={relTyp} onChange={e => setRelTyp(e.target.value)}
              style={{ padding: '6px 8px', background: 'rgba(8,13,5,0.7)', color: '#CDD4AA', border: '1px solid rgba(200,150,32,0.2)', borderRadius: 5, fontSize: 10, outline: 'none' }}>
        ...
        <div>
          ...
          {history.map(w => (
            <div key={w.id} style={{ display: 'flex', gap: 8, padding: '5px 8px', background: 'rgba(0,0,0,0.2)', borderRadius: 5, marginBottom: 3, border: '1px solid rgba(200,150,32,0.05)' }}>
              <div style={{ flex: 1, color: '#5A6840', fontSize: 9 }}>{w.nazwa_atakujacej} <span style={{ color: '#F87171' }}>{w.punkty_atakujaca}</span> vs <span style={{ color: '#60A5FA' }}>{w.punkty_broniac}</span> {w.nazwa_broniacej}</div>
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      {q ? (
        <Card color='#22C55E' glow>
          ...
          <div style={{ color: '#CDD4AA', fontWeight: 'bold', fontSize: 13, marginBottom: 4 }}>{q.nazwa}</div>
          <div style={{ color: '#7A8A5A', fontSize: 9, marginBottom: 10 }}>{q.opis}</div>
          <ProgressBar pct={pct} color='#22C55E' height={8} />
          <div style={{ display: 'flex', justifyContent: 'space-between', color: '#5A6840', fontSize: 8, marginTop: 5, marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div>
              ...
              <select value={form.typ} onChange={e => setForm(p => ({ ...p, typ: e.target.value }))}
                style={{ width: '100%', padding: '6px 9px', background: 'rgba(8,13,5,0.7)', color: '#CDD4AA', border: '1px solid rgba(200,150,32,0.2)', borderRadius: 5, fontSize: 10, outline: 'none' }}>
...
function BuffsTab({ guild, onUpdate, flashMsg }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ color: '#5A6840', fontSize: 9, marginBottom: 4 }}>Bonusy gildii działają na wszystkich członków jednocześnie.</div>
        ...
        return (
          <Card key={key}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  <span style={{ color: '#E8D070', fontWeight: 'bold', fontSize: 11 }}>{name}</span>
                  <span style={{ color, fontWeight: 'bold', fontSize: 16 }}>+{cur}%</span>
                </div>
                <ProgressBar pct={pct} color={color} />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                  <span style={{ color: '#5A6840', fontSize: 8 }}>{cur}/{max}% maks.</span>
                  {canUpgrade
...
function RankingTab() {
  ...
  if (loading) return <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#5A6840', fontSize: 10 }}>Ładowanie...</div>;

  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '36px 1fr 70px 52px 44px', gap: 6, padding: '6px 14px', borderBottom: '1px solid rgba(200,150,32,0.1)', flexShrink: 0, background: 'rgba(4,8,2,0.4)' }}>
        {['#', 'Gildia', 'EXP', 'Czł.', 'Lv'].map(h => (
          <div key={h} style={{ color: '#3A4828', fontSize: 7, textTransform: 'uppercase', letterSpacing: '1px' }}>{h}</div>
        ))}
      </div>
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {ranking.map((g, i) => (
          <div key={g.id} style={{ display: 'grid', gridTemplateColumns: '36px 1fr 70px 52px 44px', gap: 6, padding: '8px 14px', borderBottom: '1px solid rgba(200,150,32,0.04)', background: i < 3 ? `rgba(200,150,32,${0.06 - i * 0.015})` : 'transparent' }}>
            <div style={{ color: i === 0 ? '#FCD34D' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : '#5A6840', fontWeight: 'bold', fontSize: i < 3 ? 16 : 11, lineHeight: 1 }}>
              {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : i + 1}
            </div>
            <div>
              <div style={{ color: '#CDD4AA', fontWeight: 'bold', fontSize: 10 }}>[{g.tag}] {g.nazwa}</div>
              {i < 3 && pill('#C8940A', RANGA_LABEL.mistrz.replace('★ ', '') + ': ' + g.mistrz_nazwa)}
            </div>
            <div style={{ color: '#E8D070', fontSize: 9, alignSelf: 'center' }}>{fmtNum(g.gildia_exp || 0)}</div>
            <div style={{ color: '#4ADE80', fontSize: 9, alignSelf: 'center' }}>{g.member_count}</div>
            <div style={{ color: '#C8940A', fontSize: 9, alignSelf: 'center', fontWeight: 'bold' }}>Lv{g.lvl}</div>
          </div>
        ))}
      </div>
    </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            {[['Min. drużyna', raid.min_czlonkow + ' os.'], ['Uczestnicy', `${uczestnicy.length}/${raid.min_czlonkow}`], ['Pula złota', fmtNum(raid.nagroda_gold) + 'g'], ['Pula EXP', fmtNum(raid.nagroda_exp)]].map(([l, v]) => (
              <div key={l} style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 5, padding: '6px 8px' }}>
                <div style={{ color: '#5A6840', fontSize: 8 }}>{l}</div>
                <div style={{ color: '#CDD4AA', fontWeight: 'bold', fontSize: 11 }}>{v}</div>
              </div>
            ))}
          </div>
          <ProgressBar pct={readyPct} color={readyPct >= 100 ? '#22C55E' : '#60A5FA'} height={6} />
          <div style={{ color: '#5A6840', fontSize: 8, marginTop: 4, marginBottom: 10 }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.82)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: 'rgba(4,8,2,0.6)', borderBottom: '1px solid rgba(200,150,32,0.12)', flexShrink: 0 }}>
          <IconCastle size={16} style={{ color: '#C8940A' }} />
          <span style={{ color: '#E8D070', fontWeight: 'bold', fontSize: 14 }}>Gildie</span>
          <div style={{ display: 'flex', gap: 1, marginLeft: 8 }}>
            {[['my', 'Moja gildia'], ['list', 'Lista gildii']].map(([t, l]) => (
              <button key={t} onClick={() => setMainTab(t)} style={{
                ...
                border: 'none', borderBottom: mainTab === t ? '2px solid #C8940A' : '2px solid transparent',
                cursor: 'pointer', fontSize: 9, color: mainTab === t ? '#E8D070' : '#5A6840',
        ...
        {mainTab === 'my' && (
          guild === null
            ? <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#5A6840', fontSize: 11 }}>Ładowanie...</div>
            : guild === false
              ? (
                <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
                  {view === 'info' && (
                    <div style={{ textAlign: 'center', paddingTop: 40 }}>
                      ...
                      <div style={{ color: '#5A6840', fontSize: 11, marginBottom: 20 }}>Nie należysz do żadnej gildii</div>
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                        <Btn onClick={() => setView('create')} color='#FCD34D'><IconPlus size={12} /> Załóż gildię</Btn>
                        <Btn onClick={() => { setMainTab('list'); loadList(); }} color='#C8940A'><IconUsers size={12} /> Dołącz do gildii</Btn>
                      </div>
                    </div>
                  )}
                  {view === 'create' && (
                    <div style={{ maxWidth: 420, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <Btn onClick={() => setView('info')} color='#5A6840'>Anuluj</Btn>
                      </div>
                    </div>
                  )}
                </div>
              ...
              : (
                <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                  ...
                  {/* ── LEFT SIDEBAR: nav ── */}
                  <div style={{ width: 168, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(200,150,32,0.1)', background: 'rgba(4,8,2,0.5)' }}>
                    {/* Guild mini-card */}
                    <div style={{ padding: '12px 12px 8px', borderBottom: '1px solid rgba(200,150,32,0.08)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ color: '#E8D070', fontSize: 10, fontWeight: 'bold', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{guild.nazwa}</div>
                        </div>
                      </div>
                      {/* EXP micro-bar */}
                      <ProgressBar pct={((guild.gildia_exp || 0) % 10000) / 100} color='#C8940A' height={3} />
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                        <span style={{ color: '#5A6840', fontSize: 7 }}>Lv{guild.lvl}</span>
                      ...
                      {TABS.map(({ id, Icon, label }) => {
                        ...
                        return (
                          <button key={id} onClick={() => setInnerTab(id)} style={{
                            ...
                            <span style={{ color: active ? '#C8940A' : '#4A5A30', flexShrink: 0 }}><Icon size={13} /></span>
                            <span>{label}</span>
                            {active && <span style={{ marginLeft: 'auto', color: '#C8940A', flexShrink: 0 }}><IconChevronRight size={10} /></span>}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(200,150,32,0.08)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                      {(() => { const t = TABS.find(t => t.id === innerTab); return t ? <><t.Icon size={14} style={{ color: '#C8940A' }} /><span style={{ color: '#C8940A', fontWeight: 'bold', fontSize: 11 }}>{t.label}</span></> : null; })()}
                      <div style={{ marginLeft: 'auto', display: 'flex', gap: 5, color: '#5A6840', fontSize: 8 }}>
                        {pill(RANGA_COLOR[guild.myRanga] || '#CDD4AA', RANGA_LABEL[guild.myRanga])}
                      </div>
                    </div>
                    {/* Content */}
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {mainTab === 'list' && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px' }}>
            {guildList.length === 0
              ...
              : guildList.map(g => (
                <div key={g.id} style={{ padding: '12px 14px', background: 'rgba(4,10,22,0.5)', border: '1px solid rgba(59,130,246,0.12)', borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: g.otwarta ? 0 : 8 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                        ...
                        <span style={{ color: '#CDD4AA', fontWeight: 'bold', fontSize: 12 }}>{g.nazwa}</span>
                        {pill(g.otwarta ? '#22C55E' : '#F87171', g.otwarta ? '🔓 Otwarta' : '🔒 Zamknięta')}
                      </div>
                      <div style={{ color: '#5A6840', fontSize: 8, marginTop: 3 }}>Mistrz: {g.mistrz_nazwa} · {g.czlonkowie} członków · Lv{g.lvl}</div>
                      {g.opis && <div style={{ color: '#374151', fontSize: 8, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.opis}</div>}
                    </div>
                    <Btn onClick={() => join(g.id)} color='#C8940A'><IconChevronRight size={11} /> Dołącz</Btn>
                  </div>
                  {!g.otwarta && (
